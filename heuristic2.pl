...
no_surrounding_pit(X, Y) :- neighbors([X, Y],[A, B]), has_pit(A, B, no).
no_surrounding_wumpus(X, Y) :- neighbors([X, Y],[A, B]), has_wumpus(A, B, no)).
...
has_pit(X, Y, yes) :- breeze_at(A, B), isAdjacent([X, Y],[A, B]), no_surrounding_pit(A, B), asserta(pit_at(X,Y)).
...
has_wumpus(X, Y, yes) :- stench_at(A, B), isAdjacent([X, Y],[A, B]), no_surrounding_wumpus(A, B).
...
safest_room(X, Y) :-
  %Calculate all neighbors cost and select the minimun
